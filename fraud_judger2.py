...
MODEL_FILES_PATH='/Users/elex-test/Downloads/allnetworks/'


#files

RECORD_FILE='resci08a.csv'
...
resee=pd.read_csv(os.path.join(MODEL_FILES_PATH,DETAILS)).set_index('predict')
s1=joblib.load(ST_SC)
k1=joblib.load(CLUS)
...
def belongs_to_clusters(tmp,current_date):
    ...
    cluster_reason[cluster_value]=kmcluster['predict'].map(
            type_E+resee['sta1'].astype(int).astype(str)+ " other fraudulent users.")
...
#    fraud_detector(sys.argv[1],sys.argv[2])
    
    current_date='2017-08-31'
    input_file=os.path.join(MODEL_FILES_PATH,'result_predict_20180208_data_new1.csv')
    output_file=os.path.join(MODEL_FILES_PATH,'checker_result_new1.csv')
    
    import timeit
    start = timeit.default_timer()
    fraud_detector(current_date, input_file, output_file)
